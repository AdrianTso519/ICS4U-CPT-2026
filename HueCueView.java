...
public class HueCueView implements ActionListener, MouseMotionListener, MouseListener{
	...
	int ColumnClick = -100;
	int RowClick = -100;
	JFrame theFrame = new JFrame("Hues & Cues");
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if(evt.getSource() == theSecondsTimer){
			...
			if(model.intGameState != 0){
				if(blnHost){
					// If you are a host, add one to the local variable
					intReadyNumber++;
					if(intReadyNumber == model.intPlayerCount - 1){
						// If the number of players who clicked = all players - 1 (except cue giver)
						// Then all timers will be stopped, and the game moves on to the next state
						theGameTimer.stop();
						theScoreTimer.stop();
						theSecondsTimer.stop();
				...
				// If the game is done, ready button is turned into a leave button for users to leave the game 
				theReset();
			}
			...
			if(blnStateLocked){
				// if a previous timer is already done for some reason, return to ensure no further code will be triggered
				return;
			} 
			blnStateLocked = true;
			theGameTimer.stop();
			if(model.intGameState != 0){
				// If game is not done, then switch to the next state once the guessing phase is done
				model.nextState(Socket);
				stateChanges();
			}else{
				// If game is done (state == 0), reset everything used in the game
				Socket.sendText("<RESET>");
				theReset();
			}
			...
			model.nextState(Socket);
			// Move on to the next phase when the 10 second scoring phase is over
			stateChanges();
			...
			// Prevents sending the message if the field is empty
			if (!waitChatField.getText().trim().equals("")) {
				// Only sends if an actual connection is present
				if (Socket != null) {
					// The first message sent in the cue giving phase if you are the cue giver will be modified
			...
			// Do not connect if the IP field is blank
			...
			if(strLine.startsWith("<JOIN>")){
				...
				// Only the host stores the ready count
				if(blnHost == true){
					// Do not store if this stage is already reached previously
					if (blnStateLocked) {
						return;
					}
				...
				// All clients store the number of player in thr room
				...
				// All clients exit the room
				theReset();
			}else if(strLine.startsWith("<DISCONNECT>")){
				// Only the host remove the name of the user who left
				if(blnHost == true){
			...
			// --- NETWORK COMMAND DETECTION ---
				...
				theReady.setText("Ready");
				//theTimer.start(); // Start your 60 FPS repaint loop
			}else if(strLine.equals("<CLOSE>")){
				// All clients move back to the main menu if game havent started yet
				theBack();
			}else if(strLine.startsWith("<PLAYERS>")){
				// All clients load usernames & player numbers sent from the host when game starts
				...
				// All clients will know which player is giving out the cue
				model.intCueGiver = Integer.parseInt(strLine.substring(6,7));
				if(model.intCueGiver == model.intPlayerNumber){
					// You are the cue giver if it matches your own player number
					model.blnCueGiver = true;
				}else{
					model.blnCueGiver = false;
				}
			}else if(strLine.startsWith("<STATE>")){
				// All clients saves the current game state
				model.intGameState = Integer.parseInt(strLine.substring(7,8));
				stateChanges();
			}else if(strLine.startsWith("<TIMERSTART>")){
				// All clients start the timer as notified by the host
				intCountdown = Integer.parseInt(strLine.substring(12));
				theSecondsTimer.start();
			}else if(strLine.equals("<CUE1>") || strLine.equals("<CUE2>")){
				// All clients will be notified to guess the hue
				waitChatArea.append("<SYSTEM> You have 30 seconds to guess the Hue!\n");
				waitChatArea.setCaretPosition(waitChatArea.getDocument().getLength());
			}else if(strLine.startsWith("<TARGET>")){
				// If you are not the cue giver, then find your score
				if(model.intCueGiver != model.intPlayerNumber){
					...
					// Variations of the score message based on the number of points you get
					if(model.getScore(model.intRandomTile, RowClick, ColumnClick) != 0){
				...
				// The cue giver gets points based on number of players scored
				if(model.intCueGiver == model.intPlayerNumber){
					String strKey = (strLine.substring(10, 11));
					// Store the number of incoming messages
					intTempScoreCount += 1;
					if(strKey.equals("Y")){
						// +1 point if the user scored
						model.intMyScore += 1;
						intRoundScore += 1;
					}
					// If the number of incoming messages matches all players count - 1 (except cue giver)
					// Then display the score you got for the round
					if(intTempScoreCount == model.intPlayerCount - 1){
						if(intRoundScore != 0){
							if(intRoundScore == 1){
				...
				// Save other player's clicked tile for display
				...
				// Save other player's scores for display
				...
				// If the message isn't any of the above back end message, then put it on the text area
			...
			theFrame.repaint();
			hostConnect();
			// Save your own username as player 1
			model.strUserName[1] = model.username;
			model.intPlayerCount = 1;
			// Display your own port + ip
			...
			// Sends the user to the help panel
			...
			// Sends the user to the about panel
			...
			if(theWaitPanel.isShowing() && this.blnHost == true){
				// If the host pressed back while its still in the waiting room
				// Then tell all clients to move back to the main menu
				Socket.sendText("<CLOSE>");
			}
			if(this.Socket != null && Socket.connect() == true){
				// If the socket is connected, then disconnect
				Socket.disconnect();
			}
			// Reset the page you are on on the help panel
			model.intHelpCnt = 0;
			theBack();
	...
	public void stateChanges(){
		// When theese following code is triggered, next stage can be triggered now
		// So set state locked to false
		...
		// First cue giving state
		if(model.intGameState == 1){
			intReadyNumber = 0;
			if(blnHost && model.intCueGiver == 1){
				// Only add the numner of rounds if we are on the host again
				model.intRounds += 1;
				if(model.intRounds > model.intMaxRounds){
					// If max round is reached, display the winner and start a 30 second timer
					// before moving everyone back to the main menu
					...
					waitChatArea.setCaretPosition(waitChatArea.getDocument().getLength());
					model.intGameState = 0;
					Socket.sendText("<STATE>0");
					// Ready button is now a Leave button
			...
			waitChatArea.setCaretPosition(waitChatArea.getDocument().getLength());
			theGamePanel.removeMouseListener(this);
			// If the user is a cue giver
			if(model.intCueGiver == model.intPlayerNumber){
				model.blnCueGiven = false;
				// Generate a random tile as the target tile
			...
			theReady.setEnabled(false);
		}else if(model.intGameState == 2 || model.intGameState == 4){
			// Only reached once a cue has been given
			intReadyNumber = 0;
			theCountdown.setText("--");
			if(model.intCueGiver != model.intPlayerNumber){
				// All guessers can now interact with the board
				theGamePanel.addMouseListener(this);
				theReady.setEnabled(true);
			}
		}else if(model.intGameState == 3){
			// Second cue giving phase is reached once the ready count reaches the threshold
			// Or if the timer is done
			intReadyNumber = 0;
			theReady.setEnabled(false);
			waitChatArea.append("\n<SYSTEM> Player "+model.strUserName[model.intCueGiver]+" is now giving the second Cue\n");
			waitChatArea.setCaretPosition(waitChatArea.getDocument().getLength());
			theGamePanel.removeMouseListener(this);
			if(model.intCueGiver == model.intPlayerNumber){
				// Tells the cue giver to give a second cue
				model.blnCueGiven = false;
			...
			// Scoring phase is reached once the ready count reaches the threshold
			// Or if the timer is done
			theCountdown.setText("--");
			// All users will not be able to interact with the board
			theGamePanel.removeMouseListener(this);
			theReady.setEnabled(false);
			if(model.intCueGiver == model.intPlayerNumber){
				char chrLetter = (char) ('A' + model.intRandomTile[0] - 1);
				//Socket.sendText("<SYSTEM> The target tile is: "+chrLetter+model.intRandomTile[1]+"\n");
				// Cue giver send the target tile to all users
				// Users will then use the info to get their points gotten
				Socket.sendText("<TARGET>"+model.intRandomTile[0]+","+model.intRandomTile[1]);
				theScoreTimer.start();
				intCountdown = 10;
				theCountdown.setText(""+intCountdown);
				theSecondsTimer.start();
				Socket.sendText("<TIMERSTART>10");
			}else{
				// If not, send the user's clicked tile to all other users for display
		...
		}else if(model.intGameState == 0){
			// If game is done, change the ready button to a leave button
			theReady.setText("Leave");
			theReady.setEnabled(true);
		}
	}
	...
	public void showLabels(int intPlayerCount){
		...
		// Set the appropriate number of labels to be visible
		// ex: if there are 5 players, you would only want labels for P1-P5 to be shown, not P6
	...
	public void showUserNameLabels(int intPlayerCount){
		// Set the text of user name labels
		JLabel[] nameLabels = { theP1Name, theP2Name, theP3Name, theP4Name, theP5Name, theP6Name };
		for (int intCount = 0; intCount < 6; intCount++) {
	...
	public void showUserScoreLabels(int intPlayerCount){
		// Set the text of user score labels
		JLabel[] scoreLabels = { theP1Points, theP2Points, theP3Points, theP4Points, theP5Points, theP6Points };
		for (int intCount = 0; intCount < 6; intCount++) {
	...
	public void theStart(){
		...
		if(theUserNameInput.getText().trim().isEmpty() || theUserNameInput.getText() == null){
			// If host started without username input, default the name to "Host"
			model.username = "Host";
	...
	public void theReset(){
		...
		for(int intCount = 1; intCount <= 6; intCount++){
			model.strUserName[intCount] = null;
		}
		for(int intCount = 1; intCount <= 6; intCount++){
	...
	// Host Menu
		...
		theStart.setVisible(true); 
		theStart.setEnabled(false); // Disable it initially until 3 players are present
